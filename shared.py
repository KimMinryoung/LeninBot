...
def create_task_in_db(
    ...
    try:
        rows = db_query(
            "INSERT INTO telegram_tasks (user_id, content, parent_task_id, depth) "
            "VALUES (%s, %s, %s, %s) RETURNING id",
            (user_id, tagged_content, parent_task_id, depth),
        )
        task_id = rows[0]["id"] if rows else "?"
        logger.info("[shared] Task created: id=%s, priority=%s, depth=%d, parent=%s",
                     task_id, priority, depth, parent_task_id)
