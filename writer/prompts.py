...
def build_base_system_prompt(project: dict) -> str:
    ...
    return (
        ...
        "- The emotional line must turn too, not run straight: within a scene, let feeling contradict or "
        "betray itself at least once — unease inside joy, relief that curdles, self-deception talking louder "
        "as dread grows. A character who feels one thing from first line to last is a flat scene no amount "
        "of fine prose can save; find the counter-current and dramatize it.\n"
        "- A scene is also a chord in the whole work: before drafting, know what it plants or pays off "
        "(check 'Threads & setups' / the outline). The strongest scenes carry dramatic irony — the reader "
        "hearing a note the character cannot — rather than existing as self-contained vignettes.\n"
        ...
        "lives, where the emotional line turns against itself, what the scene plants or pays off in the larger "
        "design, what image or gesture carries the subtext, where it should end. Never put the plan in your reply.\n"
...
def build_diagnosis_system_blocks(project: dict) -> list[dict]:
    ...
    base = (
        ...
        "stated feeling?). Check the EMOTIONAL line separately: does feeling reverse or contradict itself at "
        "least once (기쁨 속의 불안, 자기기만), or does it run straight (기쁨→기쁨→기쁨)? A linear emotional "
        "line with polished sentences is still a flat scene — name it.\n"
        ...
        "6. 연속성·설계 — contradictions with the excerpt context or the background documents (verify with "
        "read_document when suspicious). Also judge the passage against the work's larger design: read "
        "'Threads & setups' / 'Story so far' / the outline when they exist — does this passage plant or pay "
        "off anything, does it carry dramatic irony the whole work has earned, or is it a self-contained "
        "vignette connected to nothing? An isolated well-made scene is a note, not music.\n\n"
        "# Method\n"
