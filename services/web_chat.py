...
- Persona tools and provider invocation
- SSE streaming via on_progress callback → asyncio.Queue
- Detached run lifecycle and persistence orchestration
"""
...
# Persona definitions and prompt rendering live in services/web_personas.py.
# The default prompt helper is also used by runtime inspection and tests.
...
_WEB_COMMULINGO_PEOPLE_DESCRIPTION = (
    "Read-only CommuLingo dictionary lookup "
    "(cyber-lenin.com/commulingo/people): 1,200+ Soviet/communist-history "
    "figures plus glossary terms, historical events, office timelines — "
    "bilingual. Use FIRST for such topics: search(q) matches all categories; "
    "a single match returns its full record, multiple return summaries for "
    "get_person/get_term/get_event/get_office."
)
...
class _PreparedWebChat:
    original_message: str
    regeneration_source: dict | None
    history: list[dict]
    profile: RuntimeProfile
    system_prompt: str
    feedback_ids: list[int]
    preflight_tool_detail: str


async def _prepare_web_chat(
    message: str, *, spec, session_id: str, fingerprint: str, fps: list[str],
    authenticated_user_id: int | None, regenerate_from_id: int | None,
    tone_feedback: str, feedback_note: str,
) -> _PreparedWebChat | None:
    """Resolve scoped history and feedback into one provider-ready turn."""
    persona = spec.id
    ...
    # Resolve webchat settings with the active persona's provider/tier pins.
    ...
    try:
        ...
        kg_recall_context = ""
    runtime_parts.extend(part for part in (
    ...
    return _PreparedWebChat(
        original_message=original_message, regeneration_source=regeneration_source,
        history=history, profile=profile, system_prompt=system_prompt,
        feedback_ids=feedback_ids, preflight_tool_detail=preflight_tool_detail,
    )


async def _invoke_web_model(
    turn: _PreparedWebChat, *, tools: list[dict], tool_handlers: dict,
    on_progress, budget_tracker: dict,
) -> dict:
    """Adapt the selected provider and normalize its result at this boundary."""
    profile = turn.profile
    provider = profile.provider
    loop_kwargs = dict(
        tools=tools, tool_handlers=tool_handlers, system_prompt=turn.system_prompt,
        max_rounds=profile.max_rounds, max_tokens=profile.max_tokens,
        budget_usd=profile.budget_usd, on_progress=on_progress,
        continue_on_length=True, max_length_continuations=2, budget_tracker=budget_tracker,
    )
    if provider in {"openai", "kimi"}:
        ...
        result = await openai_chat(
            turn.history, client=client, model=profile.model_id, **loop_kwargs,
        ...
        def fallback():
            from llm.openai_tool_loop import chat_with_tools as openai_chat
            return openai_chat(
                turn.history, client=_openai_client, model=failover_model, **loop_kwargs,
                return_metadata=True, provider_label="openai:web-failover",
            )

        result = await run_with_provider_failover(
            primary, fallback if failover_model else None,
            primary_label="deepseek", fallback_label=failover_model or "openai",
            budget_tracker=budget_tracker, on_progress=on_progress,
        )
    else:
        result = await chat_with_tools(
            turn.history, client=_claude, model=profile.model_id, **loop_kwargs,
        )
    return _normalize_web_result(result, budget_tracker)


def _persist_web_answer(
    turn: _PreparedWebChat, answer: str, tool_work_details: list[str], *,
    session_id: str, fingerprint: str, user_agent: str, ip_address: str,
    persona: str, authenticated_user_id: int | None, request_id: str,
    reserved_chat_log_id: int | None,
) -> int:
    """Persist a completed turn or raise; only saved answers may finish a run."""
    documents_count, web_search_used, strategy = _summarize_tool_usage(tool_work_details)
    fields = dict(
        bot_answer=answer, documents_count=documents_count, web_search_used=web_search_used,
        strategy=strategy, tool_trace=_build_tool_trace(tool_work_details), request_id=request_id,
    )
    source = turn.regeneration_source
    if source:
        chat_log_id = _update_chat_answer(
            chat_log_id=int(source["id"]), fingerprint=source["fingerprint"],
            route=f"{turn.profile.provider}_loop_regenerated", **fields,
        )
    else:
        chat_log_id = _log_chat(
            session_id=session_id, fingerprint=fingerprint, user_agent=user_agent,
            ip_address=ip_address, user_query=turn.original_message,
            route=f"{turn.profile.provider}_loop", persona=persona,
            authenticated_user_id=authenticated_user_id, reserved_chat_log_id=reserved_chat_log_id,
            feedback_ids=turn.feedback_ids, **fields,
        )
    if chat_log_id is None:
        raise RuntimeError("Web chat answer persistence failed")
    return chat_log_id
...
async def handle_web_chat(
    ...
    turn = await _prepare_web_chat(
        message, spec=spec, session_id=session_id, fingerprint=fingerprint, fps=fps,
        authenticated_user_id=authenticated_user_id, regenerate_from_id=regenerate_from_id,
        tone_feedback=tone_feedback, feedback_note=feedback_note,
    )
    if turn is None:
    ...
    profile = turn.profile
    provider = profile.provider
    regeneration_source = turn.regeneration_source
    ...
    budget_tracker: dict = {}
    if turn.preflight_tool_detail:
        budget_tracker["tool_work_details"] = [turn.preflight_tool_detail]
    ...
    async def _run_llm():
        try:
            ...
            metadata = await _invoke_web_model(
                turn, tools=web_tools, tool_handlers=web_handlers,
            ...
            chat_log_id = await asyncio.to_thread(
                _persist_web_answer, turn, answer, tool_work_details,
                session_id=session_id, fingerprint=fingerprint, user_agent=user_agent,
                ip_address=ip_address, persona=persona, authenticated_user_id=authenticated_user_id,
                request_id=web_request_id, reserved_chat_log_id=audit_chat_log_id,
            )
