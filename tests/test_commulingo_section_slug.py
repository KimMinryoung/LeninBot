...
class SectionSlugTests(TestCase):
    def test_registry_generates_distinct_topic_slug_and_tracks_cost(self):
        ...
        with patch('commulingo.section_slug.resolve', return_value=profile), \
             patch('commulingo.section_slug.generate_detailed', return_value=result) as call, \
             patch('commulingo.section_slug.estimate_cost_usd', return_value=.0001):
    ...
    def test_unusable_or_failed_model_slug_falls_back_to_heading(self):
        ...
        with patch('commulingo.section_slug.resolve', return_value=profile), \
             patch('commulingo.section_slug.generate_detailed') as call:
    ...
    def test_model_slug_is_folded_before_validation(self):
        ...
        with patch('commulingo.section_slug.resolve', return_value=profile), \
             patch('commulingo.section_slug.generate_detailed', return_value=result):
    ...
    def test_fallback_trims_long_and_korean_only_headings(self):
        ...
        with patch('commulingo.section_slug.resolve', return_value=profile), \
             patch('commulingo.section_slug.generate_detailed', return_value=failed):
    ...
    def test_existing_heading_never_calls_model(self):
        with patch('commulingo.section_slug.generate_detailed') as call:
...
class SectionApiTests(IsolatedAsyncioTestCase):
    async def test_create_generates_slug_after_read_and_update_keeps_existing_slug(self):
        from commulingo.people import _exec_commulingo_section_save
        current = {'sections':[{'slug':'early-life'}]}
        with patch('commulingo.people.call_person_service', return_value=current) as read, \
             patch('commulingo.section_slug.generate_section_slug', return_value='military-career') as slugger, \
             patch('commulingo.people._exec_commulingo_write', return_value='saved') as write:
    ...
    async def test_update_without_slug_is_rejected_before_write(self):
        from commulingo.people import _exec_commulingo_section_save
        with patch('commulingo.people._exec_commulingo_write') as write:
            result = await _exec_commulingo_section_save('update','nikolai-pukhov',
    ...
    async def test_duplicate_heading_on_create_returns_error_without_write(self):
        ...
        with patch('commulingo.people.call_person_service', return_value=current), \
             patch('commulingo.section_slug.generate_detailed') as call, \
             patch('commulingo.people._exec_commulingo_write') as write:
            result = await _exec_commulingo_section_save('create','nikolai-pukhov',
