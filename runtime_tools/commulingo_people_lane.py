"""The CommuLingo people lane: card selection, curator prompts and one run.
...
# Same logger name as before the move, so journal lines keep their source.
...
CARD_STYLE_GUIDANCE = (
    ...
    "- The `cyrillic` field is the NATIVE-SCRIPT name line, not literally Cyrillic: write "
    "the person's name in the script their own nation uses — 김무정 for a Korean, 彭德怀 for "
    "a Chinese, 'Võ Nguyên Giáp' for a Vietnamese, 'Jean Jaurès' for a French figure. A "
    "Russian transliteration of a non-Russian name is rejected, and OMITTING the field to "
    "dodge that rejection is a defect, not a fix: rewrite it in the right script in the "
    "same turn.\n"
    "- Submit givenName/familyName as separate fields; the save derives the display name "
    "and its order from citizenship — Korean/Chinese/Vietnamese names render family-first "
