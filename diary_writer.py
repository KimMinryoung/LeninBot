...
## 최근 일기 요약 (중복 방지용 — 이미 다룬 주제는 피하고 새로운 관점을 제시하라)
{prev_ref}
...
5. 이전 일기들과 내용이 겹치지 않도록 새로운 소재나 시각을 우선할 것
5. 한국어로 작성
...
def _generate_diary(chat_logs: list[dict], news: str, previous_diaries: list[dict]) -> tuple[str, str] | None:
    ...
    # Previous diaries summary (최근 5건)
    prev_ref = ""
    if previous_diaries:
        for d in previous_diaries[:5]:
            ts = d.get("created_at", "?")
            title = d.get("title", "")
            body = d.get("content", "")[:200]
            prev_ref += f"- [{ts}] {title}: {body}\n"
    else:
        prev_ref = "(첫 번째 일기)"
