...
# fix_state_capitalism_report_citations.py.
#
# Usage:
#   bash scripts/run_fix_state_capitalism_report.sh          # dry-run
#   bash scripts/run_fix_state_capitalism_report.sh --apply  # write the corrections
#
...
if [ -z "${DB_PASSWORD:-}" ]; then
    ...
    exec sudo systemd-run --pipe --wait --collect --uid="$(id -un)" \
        ...
        "$ROOT/scripts/fix_state_capitalism_report_citations.py" $FLAG
fi

exec "$ROOT/venv/bin/python" "$ROOT/scripts/fix_state_capitalism_report_citations.py" $FLAG
