#!/usr/bin/env python3
"""Daily main DB backup to Cloudflare R2.

The main database (RAG corpus, chat logs, tool audit, agent state) lives in
the local Docker Postgres (leninbot-pg, `leninbot` database) since the
2026-07-28 migration off Supabase, so this box is the only copy — this job
is the durability story until the standby VM lands (mirrors
backup_writer_db_to_r2.py).
...
main-db-backup-YYYY-MM-DD.dump to the cyber-lenin-backups R2 bucket
(rolling R2_RETENTION_DAYS), and keeps a rolling LOCAL_RETENTION_DAYS copy
under data/main_db_backups/.

Restore (into a fresh or wiped database):
  docker exec -i leninbot-pg pg_restore -U postgres -d leninbot \
...
_LOCAL_KEY_RE = re.compile(r"^main-db-backup-(\d{4}-\d{2}-\d{2})\.dump$")
...
def _dump(path: str) -> None:
    """pg_dump the main DB through the container (version-matched tools)."""
    with open(path, "wb") as out:
        subprocess.run(
            ["docker", "exec", CONTAINER, "pg_dump", "-U", "postgres", "-d", "leninbot", "-Fc"],
            stdout=out,
            check=True,
            timeout=600,
        )
...
def _verify(path: str) -> int:
    ...
    if not any("lenin_corpus" in l for l in entries):
        raise RuntimeError("archive verification failed: lenin_corpus missing from TOC")
    return len(entries)


def main() -> int:
    today = datetime.now(KST)
    archive_key = f"main-db-backup-{today.strftime('%Y-%m-%d')}.dump"
    backup_dir = ROOT / "data" / "main_db_backups"
