...
#
# Usage:
#   bash scripts/run_commulingo_register.sh scripts/register_gotha_utopian_people.py
#   bash scripts/run_commulingo_register.sh scripts/register_gotha_utopian_people.py --apply
#
...
if [ $# -lt 1 ]; then
    echo "Usage: $0 <script.py> [flags…]" >&2
    exit 2
fi
SCRIPT="$1"
shift
...
if [ -z "${DB_PASSWORD:-}" ]; then
    ...
    exec sudo systemd-run --pipe --wait --collect --uid="$(id -un)" \
        ...
        "$ROOT/venv/bin/python" "$ROOT/$SCRIPT" "$@"
fi

exec "$ROOT/venv/bin/python" "$ROOT/$SCRIPT" "$@"
