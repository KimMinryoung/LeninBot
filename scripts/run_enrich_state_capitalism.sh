...
# enrich_state_capitalism_term.py.
#
# Usage:
#   bash scripts/run_enrich_state_capitalism.sh          # dry-run
#   bash scripts/run_enrich_state_capitalism.sh --apply  # write the term
#
...
if [ -z "${DB_PASSWORD:-}" ]; then
    ...
    exec sudo systemd-run --pipe --wait --collect --uid="$(id -un)" \
        ...
        "$ROOT/scripts/enrich_state_capitalism_term.py" $FLAG
fi

exec "$ROOT/venv/bin/python" "$ROOT/scripts/enrich_state_capitalism_term.py" $FLAG
