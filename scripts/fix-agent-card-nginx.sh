#!/usr/bin/env bash
# Fix: proxy A2A endpoints (/.well-known/agent-card.json, /a2a) to API (port 8000) on cyber-lenin.com
set -euo pipefail

CONF="/etc/nginx/sites-enabled/leninbot-frontend"

if grep -q 'well-known/agent-card.json' "$CONF" 2>/dev/null; then
...
echo "Done. Testing..."
curl -s -o /dev/null -w "/.well-known/agent-card.json → %{http_code}\n" https://cyber-lenin.com/.well-known/agent-card.json
