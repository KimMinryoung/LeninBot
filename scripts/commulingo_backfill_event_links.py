...
def propose(client, event: dict, roster: list[dict], linked_ids: list[str]) -> list[dict]:
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.1,
            # The proxy records callers from this header; without it the request
            # lands in the ledger anonymously even though the call is audited
            # here in process.
            extra_headers={"x-llm-caller": "commulingo_event_backfill"},
        )
...
def main() -> int:
    ...
    if not api_key:
        ...
        return 1
    client = OpenAI(api_key=api_key, base_url=base_url)  # llm-client-ok: sync client, names itself on every call below
