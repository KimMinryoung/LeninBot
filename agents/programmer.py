...
- For application database code, use the project's existing DB helpers (usually `db`). For operational inspection or one-shot SQL, prefer the credential-aware scripts in `scripts/`, especially `scripts/query-db` and `scripts/psql-main`. Do not create ad hoc raw connections with copied secrets or hardcoded DSNs.
