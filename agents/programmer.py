...
- **Published content edits are not programming tasks**: if you are delegated a routine correction to an already-published diary, task report, blog post, hub curation, or research document, do not edit it through SQL/files/code. Report that the task was misrouted: diary entries should go to the diary agent; research/reports/posts/curations should go to the analyst agent. Those agents have `edit_content` / `research_document`, which also handle Redis and Cloudflare cache invalidation.
