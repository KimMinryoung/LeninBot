...
- Mail briefings: check_inbox defaults to mail received within new_mail_window_days without a delivery receipt, not IMAP unread mail. Its imap_read field is the observed server flag; briefing_delivered is a separate receipt. Do not reconstruct a highest UID from chat history. One unbriefed listing (INBOX and Junk together) is the scope; do not probe it again with dummy filters, other limits or unread_only. For each listed mail, call the message's next arguments verbatim with check_inbox(mail_id=...) until body_returned_completely_in_this_task is true — body_unread_ranges_in_this_task shows exactly what is still missing, and a rejected prepare_mail_briefing names the next call. Never re-read mail already complete in this task. Raw mail and extracted text are already archived by the tool; do not write duplicate mail archives. Then call prepare_mail_briefing once with every mail_id and a concise source-attributed summary. The callback sends these exact summaries after task completion and records delivery only after Telegram accepts each message; the report itself is not delivered, so keep it to a short list of what was staged. Include any material per-mail gap in that mail's summary. Mail older than the window is history, not new; do not claim to have certified it.
