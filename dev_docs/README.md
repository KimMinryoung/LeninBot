...
새 작업은 먼저 [전체 런타임·요청 경로](project_state.md)에서 서비스 소유권과 데이터 흐름을 확인한다. 아래 표에서 작업 주제에 맞는 상세 문서로 이동한 뒤, 실제 코드·설정과 대조한다. 문서는 설계의 길잡이이며 실행 코드와 설치된 설정의 현재 상태를 대신하지 않는다. 프런트엔드와 Nginx 설정의 원본은 별도 저장소 `/home/grass/frontend`에 있다.
