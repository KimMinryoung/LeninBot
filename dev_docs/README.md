...
- Python 변경을 운영에 적용하기 전에 `scripts/run_unit_tests.sh`를 실행한다. 이 명령은 테스트 전에 `scripts/check_python_names.py`로 저장소의 Python 파일에서 미정의 이름과 문법 오류를 검사한다. 의존성은 `requirements.txt`의 `pyflakes`다. 테스트는 stdlib unittest로 돌리고, `pytest`를 import하는 모듈(모듈 수준 테스트 함수는 unittest가 실행하지 않는다)은 pytest로 한 번 더 돌린다. import 대상의 실제 존재 여부와 실행 중 동적 이름 해석은 이 검사 범위 밖이므로, 변경한 서비스의 모듈 import와 관련 실행 경로도 확인한다.
