...
- `leninbot-api.service` may mount `writer_anthropic_api_key` for the `/writer` personal fiction workspace. `creative_writer.py` checks `WRITER_ANTHROPIC_API_KEY` first and falls back to `ANTHROPIC_API_KEY`, so a separate paid Claude Fable 5 key can be isolated from Cyber-Lenin's normal Claude usage. `WRITER_ACCESS_KEY` can protect direct writer API calls separately; if unset, `/writer/*` accepts `ADMIN_API_KEY` through `X-Writer-Key`. The public frontend writer page normally uses the admin login session and server-side key injection instead of exposing either key to the browser.
