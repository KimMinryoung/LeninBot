...
`upsert_identity_*` unions ids/aliases into an existing node (and fills an empty summary); `merge_entity_nodes_*` is the one merge implementation (edges moved with UUID/provenance/validity preserved, MENTIONS moved, summary filled, identity unioned, duplicate DETACH DELETEd) — `skills/kg-maintenance/scripts/merge_entities.py` and `kg_runtime.admin.kg_merge_entities` delegate to it. After every free-text episode, `post_episode_merge` folds new nodes into same-label nodes that share an alias key across groups (graphiti only searches resolution candidates inside the episode's own `group_id`, which is how one entity became five nodes).
...
**Alias matching rules** (`AliasIndex.match`, used by search auto-mode, document mentions and recall): Latin keys match as whole tokens (≥4 chars, or a full multi-word phrase); Korean keys must start a word and may only be followed by a particle/copula or the word end (`_KO_PARTICLES`) — so `우리가`/`리델리가` do not hit 리가, `선언했다` does not hit the weak alias 선언 of 공산당 선언, and `스탈린주의` does not hit 스탈린. Keys in `GENERIC_ENTITY_NAMES` are never indexed. Before 2026-09-03 Korean keys were plain substrings, and the web-chat recall block injected Riga into fantasy-worldbuilding chats.
...
- **Publish hook**: `publishing/research.py` schedules `doc_extract.extract_research_by_slug` after a public publish/edit (fire-and-forget; the nightly job is the backstop).
...
**Entity-gated recall** (`KG_ENTITY_GATED_RECALL=1`, default off): `kg_runtime.recall.entity_gated_kg_block(text, provider)` renders a `<knowledge-graph>` block (Markdown for non-Claude) for up to two entities named in a message/task — alias match (`broad=False`) + neighbourhood, zero LLM/embedding cost. `Reference` edges (document mentions) never appear in the block and a hit with nothing else is skipped — before 2026-09-04 four of six recalled lines were "문서 X에 언급된다". Injected next to the experience block in `telegram/commands.py`, `telegram/tasks.py` and `services/web_chat.py`. The durable task worker queries both recall paths using the original commissioned task text; dependency/synthesis results are separate context and never replace that query. The same flag lets `commulingo_people get_person` append `kg_facts` (non-mirror facts attached to the person's node).
...
- Backfilled summaries exclude expired/invalidated and `Reference` edges, rank by trust then newest date,
  and carry `KG 관계 발췌:` plus `summary_source=kg_facts`, `summary_fact_uuids` and `summary_generated_at`.
  They are derived KG excerpts, not independently verified summaries. Source-owned and curated nodes
  are skipped, and writes recheck that the summary is still empty. Nodes without usable facts remain empty.
- Weekly reports distinguish no `RELATES_TO` relations from no connections of any kind;
  episode-linked nodes are retained rather than deleted as disposable orphans.
- Entity merges preserve every transferred relation's UUID, provenance, sync key and validity;
  equal predicates/neighbours do not justify dropping different claims or source versions.
  Relations directly between merged nodes are removed to avoid self-loops. Verify with
  `scripts/smoke_kg_merge_preservation.py` (Neo4j transaction always rolled back).
