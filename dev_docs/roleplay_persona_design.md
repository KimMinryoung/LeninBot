...
| 긴장 | 목표값 쪽으로 시간당 최대 6씩 접근. 목표 = 위협 단계 10/40/75/90 − 습관화(safe/uncertain 구간의 누적 `calm_minutes`, 시간당 1, 최대 15) − (의지−50)/50×10 − 수면 중 10 + 통증≥50이면 5, 5–95로 제한. threatening/immediate 구간, 부상 event, 긴장을 올리는 event는 `calm_minutes`를 0으로 되돌린다. 2026-09-18 이전에는 위협 목표값만 있어 uncertain에서 40에 고정됐다 |
