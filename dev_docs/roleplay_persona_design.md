...
`/status`는 다음 시도·회피 목적을 포함한 상세 내용을 항상 표시한다. `/status 상세`도 같은 출력이다. 계획(next_action)과 완료 사건(last_event)은 구분한다.
...
| 명료함 clarity (100=또렷) | 수면 +4 × 수면의 질; 깨어 있을 때 각성 16h −1(24h −2), 통증≥75 −0.5, immediate −1, 미처치 악화 3단계 부상 −1; 피로≤60 휴식 +1. 시간 감소는 30 아래로 내리지 않음(`roleplay_game_balance.md`) |
...
#### 급성 통증 감쇠와 혼자 있는 구간의 위협 해소 (2026-09-19)

2026-09-19까지는 기저값 위의 통증이 부상 추세로만 움직여, 뺨 구타·강요 같은 event로 올린 통증이 stable 부상 아래에서
밤새 휴식해도 그대로 남았다(실제 로그: 68.4 → 7.5시간 선잠 뒤 64.7). 또 가해자가 나간 뒤 혼자 보낸 밤을 모델이
threatening으로 넘겨 의지가 −2.5/h로 0까지 소모됐다. 다음 규칙을 추가했다.
...
`/status`, `/status 상세`, `/status detail`은 모두 동일한 상세 상태를 표시한다.
수치·몸과 질병·활동과 환경·마음과 장면·예정과 기록·시간 계산 근거로 구분하고 구역 사이에 빈 줄을 둔다.
부상은 항목별로 줄을 나누며 예정 사건은 활성 항목 전체를 표시한다. 비어 있는 항목도 등록/기록 없음으로 표시한다.
현재 장면 설명과 회피 목적·다음 시도·계산 시점·시간 해석 근거·수면·위협·부상·변경 이유를 포함한다.
긴 출력은 Telegram 메시지 길이 제한에 맞춰 나눈다. 상세 명령을 다시 입력하라는 안내는 하지 않는다.
...
연기 문맥은 기존 goal/avoid/next_action/unresolved를 `scene_focus`로 묶고 현재 입력·장면과 어휘가 겹치는
거래·지키는 것·예정 단서를 최대 3개 우선 표시한다. 단순 정렬 힌트이며 장면 허가나 사건 실행 판정이 아니다.
전체 예정 사건의 시간 경계는 기존 엔진이 소유한다. 최근 이행/파기된 거래 3건도 질적 결과로 전달하여,
종료된 거래가 열린 거래 목록에서 빠진 뒤에도 다음 요구·태도에 참고할 수 있게 한다.

인물은 현재 장면 안의 질문·거절·흥정·작은 시도를 자발적으로 할 수 있다. 큰 이동·시간 건너뛰기·상대의 응답과
성공은 대신 확정하지 않는다. 약속 이행을 무조건적 신뢰로 바꾸지 않으며 평범한 대화에 갈등을 강요하지 않는다.
선택지는 사용자가 다음 행동을 고민하거나 요청할 때 제시한다. 추가 목표/자원 시스템이나 모델 호출은 없다.
이 지침은 연기 의도이며, 합성 문맥 테스트가 실제 모델의 모든 반응을 보장하지는 않는다.
