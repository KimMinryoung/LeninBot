...
| Public content | `runtime_tools/research_store.py`, `runtime_tools/site_publishing.py`, `runtime_tools/publication_records.py`, `runtime_tools/research.py`, `runtime_tools/post_edit.py`, `api_routes/private_reports.py` (JSON), frontend `/admin/private-reports` shell |
| Hub 큐레이션 (`/curate`) | `telegram/curate.py` (URL 정규화·중복 검사·태스크 등록, 쓰기 경계 검증 래퍼, 결정적 결과 DM), `agents/hub_curator.py` (DeepSeek V4.1 Flash 작성자 스펙, `CURATION_LIMITS` 단일 출처), `runtime_tools/site_publishing.py` (`publish_hub_curation` 툴, `hub_curations` 테이블) |
| CommuLingo 인물·용어 사전 | `runtime_tools/commulingo_people.py` (read + six target-specific narrow writes; shared normalization, structured errors; direct/staging switch in `config/commulingo_people.json`), `tool_gateway/profiles.py` + `agents/analyst.py` (Telegram direct/delegated narrow-write surfaces), `agents/commulingo_curator.py` + `scripts/commulingo_people_maintainer.py` / `scripts/commulingo_terms_maintainer.py` (typed discovery and stage-scoped scheduled direct maintenance; 레인 공용 설정·카운터·단계 실행기는 `runtime_tools/commulingo_lane.py`, 인물 레인 선택·프롬프트·`run_once`는 `runtime_tools/commulingo_people_lane.py`), `scripts/commulingo_suggestions.py` (staging 리뷰 CLI). 인물/절 저장·승인은 frontend 공통 서비스의 Docker RPC를 사용한다. 필수 버전·근거·검토·보강 상태는 [인물 편집 계약](commulingo_editorial.md), 데이터/렌더링은 별도 저장소 `/home/grass/frontend/dev_docs/commulingo_people_handoff.md` 참고 |
...
`shared.py` holds only `KST`, `upload_to_r2` and the static `MODULE_ARCHITECTURE` self-description; the old compatibility re-exports were removed (2026-09-24). Import from the domain modules above instead of growing `shared.py`. Per-turn runtime context (`current_task_ctx`, runtime prelude, context-block join/merge) lives in `llm/runtime_context.py` so the browser worker and A2A API do not import `telegram.bot`; `telegram.bot` re-exports the same objects. Checkout and frontend locations come from `ops/paths.py`: `PROJECT_ROOT`/`VENV_PYTHON` derive from the checkout, `FRONTEND_DIR` (default `/home/grass/frontend`) locates the separate frontend repository, and `commulingo_data_file()` keeps the per-file `COMMULINGO_*` overrides used by tests. Route modules are service boundaries only when included by a dedicated service entrypoint such as `services/novel_writer_api.py`, `services/email_api.py`, or `services/a2a_api.py`; the remaining route modules are imported into `services/api.py` and run inside `leninbot-api.service`.
...
- **루트 호환 모듈 (2026-09-24)**: `site_publishing`, `research_store`, `publication_records`(→ `runtime_tools/`), `task_store`(→ `telegram/`), `redis_state`(→ `memory_store/`), `prompt_context`, `skills_loader`(→ `llm/`), `audit_sink`(→ `ops/`), `self_modification_core`(→ `self_runtime/`), `scripts/commulingo_research_memory`·`commulingo_run`(→ `runtime_tools/`)은 패키지로 옮겼다. 루트·`scripts/`의 같은 이름 파일은 옛 import 경로용 shim이며, 루트 모듈은 `sys.modules` 별칭이라 옛 경로로 patch해도 같은 모듈에 닿는다. 새 코드는 패키지 경로를 쓴다. 라이브러리 패키지는 `scripts/`를 import하지 않는다.
