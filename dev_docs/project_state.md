...
- Services do not run startup DDL. Apply `scripts/schema_migrations.py` before deploying code that depends on new tables, columns, indexes, or constraints. (Exception to revisit: `leninbot-roleplay.service` currently creates its own `roleplay_chat_history` / `roleplay_clear_markers` tables at startup.)
