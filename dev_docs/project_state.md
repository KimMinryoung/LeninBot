...
- `/writer` is an admin-gated personal fiction workspace, separate from the public chat/persona/model-tier surfaces. It stores `writer_projects`, `writer_messages`, `writer_manuscripts`, searchable manuscript chunks, and manuscript revision metadata, then calls Claude Fable 5 directly through `creative_writer.py`. Apply `venv/bin/python scripts/schema_migrations.py --only writer-tables` before first use or after writer schema changes.
