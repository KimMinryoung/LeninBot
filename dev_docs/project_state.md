...
- Every tool call from every interface funnels through `tool_gateway.dispatcher.execute_tool`, where the **tool security gateway** (`security_gateway/*`) authorizes the call against one unified policy and writes a `tool_audit_log` row + structured journal line. Authorization/pre-check errors fail closed; successful side-effect calls are deduplicated within one provider tool loop. New owner-gating and rate-limit rules still ship in **shadow** mode by default (`gateway_enforce_mode` in `config.json`); web-chat/A2A interface restrictions are always enforced. Inspect with `scripts/security_gateway.py {policy,check,audit}`. Full design: `dev_docs/security_gateway.md`.
