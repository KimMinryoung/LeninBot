...
`shared.py` holds only `KST`, `upload_to_r2` and the static `MODULE_ARCHITECTURE` self-description; the old compatibility re-exports were removed (2026-09-24). Import from the domain modules above instead of growing `shared.py`. Per-turn runtime context (`current_task_ctx`, runtime prelude, context-block join/merge) lives in `llm/runtime_context.py` so the browser worker and A2A API do not import `telegram.bot`; `telegram.bot` re-exports the same objects. Route modules are service boundaries only when included by a dedicated service entrypoint such as `services/novel_writer_api.py`, `services/email_api.py`, or `services/a2a_api.py`; the remaining route modules are imported into `services/api.py` and run inside `leninbot-api.service`.
