...
- Deploy: `deploy.sh` → `scripts/svc deploy [--api|--telegram|--frontend|--all] [--restart]` (Telegram `/deploy`). It pulls `main`, runs `pip install` only when `requirements*` changed and `daemon-reload` when `systemd/` changed. `--all` (default) first runs `scripts/restart_guard.py` for api and telegram and restarts nothing if either has active work; it then restarts every enabled long-running unit in dependency order — `leninbot-llm-proxy`, `leninbot-web-gateway`, `leninbot-api`, `leninbot-a2a-api`, `leninbot-email-api`, `novel-writer-api`, `leninbot-roleplay`, `leninbot-browser` (if active), `leninbot-telegram` last — warns when a unit's MainPID did not change, and hands the frontend to the frontend repository's `scripts/deploy`. `--api`/`--telegram`/`--frontend` restart only that one. Deploy does **not** copy unit files into `/etc/systemd/system/` (do `sudo cp systemd/<unit> /etc/systemd/system/` + `daemon-reload` yourself).
