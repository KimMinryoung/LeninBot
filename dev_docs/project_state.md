...
leninbot-api (:8000, FastAPI)
        |-- /chat -> web_chat.py
        |-- /writer compatibility routes -> api_routes.writer (transition path)
...
| `leninbot-api.service` | `uvicorn api:app` | web chat, admin API, email/A2A/private reports; temporarily still includes `/writer/*` compatibility routes |
