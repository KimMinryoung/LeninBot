...
The CommuLingo curator binding (`bot_config.resolve_agent_tool_loop`, used by the pipeline editor/reviewer and the curator lanes) is the exception: since 2026-09-25 it sends DeepSeek tool loops to the OpenAI-compatible endpoint (`_deepseek_client` + `llm.openai_tool_loop`, `extra_body.thinking`, `max_tokens`, no `parallel_tool_calls`). The Anthropic-compatible endpoint returned `input: {}` for long editor submissions (2,600-2,900 output tokens) with no raw text to recover. `DEEPSEEK_TOOL_LOOP_PROTOCOL=anthropic` restores the old path.
