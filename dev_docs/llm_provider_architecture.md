...
최종 확인 기준: 2026-09-23 코드 트리와 새 모델의 프록시 실호출.
...
Since 2026-09-25 every DeepSeek **tool loop** — Telegram chat, background tasks, the hourly autonomous project loop, A2A (`leninbot-a2a-api.service`), public web chat, browser worker tasks, the roleplay bot, the DeepSeek writer models and the CommuLingo curator binding — takes its loop, client and options from `bot_config.deepseek_tool_loop()`, which uses DeepSeek's OpenAI-compatible endpoint (`_deepseek_client` + `llm.openai_tool_loop`, thinking in `extra_body`, `max_tokens`, no `parallel_tool_calls`, `preserve_reasoning_content` when thinking is on). The Anthropic-compatible endpoint returned `input: {}` for long tool arguments (2,600-2,900 output tokens) with no raw text to recover. `DEEPSEEK_TOOL_LOOP_PROTOCOL=anthropic` restores the previous `claude_loop` path everywhere. browser-use automation (`browser/use_agent.py`) is not one of our tool loops and still uses its own `ChatAnthropic` adapter; one-shot `messages.create` scripts are unchanged. Agent/task DeepSeek paths resolve `thinking_policy=tool_loop` through `tool_gateway.inference`; the current `DEEPSEEK_TOOL_THINKING_MODE` default is off, and operators can enable it centrally without per-call overrides. `llm/claude_loop.py` preserves `thinking` and `redacted_thinking` assistant content blocks in replayed tool-call turns so DeepSeek receives the reasoning payload it requires on follow-up requests. Public web chat and browser automation deliberately keep DeepSeek Flash in non-thinking mode (`thinking={"type": "disabled"}`): web chat does it for lower latency, while browser automation does it because browser-use relies on forced structured tool calls and DeepSeek does not support that path with thinking enabled. DeepSeek Flash now accepts image content through its Anthropic-compatible Messages endpoint (verified through the local proxy with an image input). The installed browser-use 0.12.5 still forces `use_vision=False` for DeepSeek, so this integration runs a non-vision DOM/tool controller first and retries with the configured Google/OpenAI vision fallback only if that attempt fails. Browser-use `history.is_successful()` supplies the final success flag; a completed but unsuccessful `done` action is not treated as success.

The roleplay bot (`leninbot-roleplay.service`, `roleplay/bot.py`) is a separate runtime, not the Cyber-Lenin orchestrator, and does not read `config.json`'s `provider`/`chat_model` keys. It pins the DeepSeek provider/model while routing through the proxy: `bot_config.deepseek_tool_loop()` (OpenAI-compatible endpoint), model `deepseek-flash` (via `_resolve_deepseek_model("deepseek_flash")`), with thinking **enabled** (`reasoning_effort=high`). Thinking is on for answer quality; `preserve_reasoning_content=True` keeps the reasoning in `reasoning_content` for replay and out of the user-facing reply. The bot ignores the global `DEEPSEEK_THINKING_MODE` env and sets its thinking inline.

The personal fiction writer (`/writer`, `api_routes/writer.py`, `services/novel_writer_api.py`, `writer/`) is separate from normal provider selection but not from gateway enforcement. Its role-level inference envelopes are centralized in `writer.config.WRITER_CALL_POLICIES` (input/output ceilings, rounds, output continuations, thinking policy); the heavy `main` and `revision` roles import the shared 160k/32k gateway defaults, while `diagnosis`, `line_edit`, and `research` retain smaller explicit role limits; input overflow uses durable summaries plus chapter-boundary anchor replay rather than silent truncation. It uses the shared `llm.claude_loop.chat_with_tools` path with `model="claude-fable-5-1"` by default and can route explicitly selected DeepSeek or Kimi K3 writer models: Kimi through its Anthropic-compatible client, DeepSeek through `writer.models.writer_chat`, which sends it to the OpenAI-compatible loop (system blocks joined into one prompt, thinking moved to `extra_body`). The Claude writer client uses the proxy's shared `anthropic` route and `ANTHROPIC_API_KEY`. Kimi Writer uses the proxy's Moonshot Anthropic-compatible route, model `kimi-k3`, and K3's default thinking. The dedicated process is `novel-writer-api.service` and starts after the proxy is ready.
...
Browser automation runs inside the separate `leninbot-browser.service` process. `browse_web` defaults to `mode=computer`, using OpenAI Responses' native `computer` tool in a fresh headless Playwright Chromium context: GPT-6 Luna (`tier:low`) by default or GPT-6 Sol (`tier:high`). The tool caller may supply `start_url` or a URL in the task; without one, it starts at Google Search. The action loop captures screenshots and executes mouse/keyboard requests automatically, limits model calls and run time, and returns a completion flag only when the model explicitly writes `STATUS: COMPLETED`. This mode does not use saved browser cookies or the browser-use vision fallback. It controls a browser page; full desktop apps do not yet have an action executor. The existing LLM proxy and audited OpenAI client carry all Responses calls. The older DeepSeek browser-use path remains available with explicit `mode=agent`. The outer Browser Agent worker still uses its own provider setting in `config/agent_runtime.json`.

Browser-use `mode=agent` has its own environment overrides:
...
|---|---|---|---|---|---|
| `high` | `claude-opus-5-5` alias | `gpt-6-sol` | `deepseek-flash` | `kimi-k3` | local backend model |
| `medium` | `claude-sonnet-5` alias | `gpt-6-sol` | `deepseek-flash` | `kimi-k3` | local backend model |
| `low` | `claude-haiku-4-5` alias | `gpt-6-luna` | `deepseek-flash` | `kimi-k3` | local backend model |

Claude aliases are resolved lazily through Anthropic Models API and cached in-process. OpenAI, DeepSeek, Kimi, and local model IDs resolve synchronously from `llm/provider_registry.py` or local backend config. GPT-6 Sol covers high and medium, and Luna covers low; historical `gpt56*` selector names resolve to GPT-6 for stored selections. GPT-6 agent tool loops use Responses API, which supports reasoning with function calls; the loop replays returned reasoning items with tool outputs. Browser-use's OpenAI Chat Completions integration sends `reasoning_effort=none` for GPT-6 tool calls. Kimi currently has one model in all three tiers, so the tier changes budget/display grouping but not the upstream model ID.

Direct proxy clients select a provider route and send `model="tier:high|medium|low"`; Claude/OpenAI also accept `tier:frontier`. Gemini's registry tiers are Pro 3.1 Preview (high), Flash 3.8 (medium), and Flash-Lite 3.5 (low). Claude's frontier tier is Fable 5.1, while Writer keeps its separate `fable`/`fable_fast` effort choices. `llm.provider_registry.CURRENT_TEXT_MODELS` is the one maintained catalog for those tier targets. [Provider IDs are pinned snapshots](https://platform.claude.com/docs/en/about-claude/models/model-ids-and-versions), so a future release requires updating this catalog; the gateway rejects unregistered text IDs instead of silently calling an older version. Embedding, image, and other non-text endpoints retain their own model selections.

과금 계산의 모델별 요율·캐시·장문 구간은 `llm/provider_registry.py`가 소유한다. 문서에 프로모션 가격표를 복제하지 않는다. 외부 최신 가격 검증과 저장소 계산 정책 확인은 별개다.

DeepSeek V4.1 Flash는 2026-09-10 갱신된 사용자 제공 이메일 공지와 운영 프록시의 upstream `/deepseek/models` 응답을 근거로 적용했다. 실제 모델 목록은 `deepseek-flash`, `deepseek-v4-pro`이며 정식 Flash ID는 `deepseek-flash`다. 이후 [공식 출시 공지](https://api-docs.deepseek.com/updates/#deepseek-v41-flash-release)에서도 ID와 출시를 확인했다. 사용자 요청에 따라 DeepSeek의 모든 티어와 기존 `deepseek_pro`/`deepseek_flash` 선택 별칭을 `deepseek-flash`로 해석한다. 에이전트의 저장된 Pro 별칭도 즉시 Flash를 사용하며, 원샷 번역·브라우저·Writer의 직접 모델 ID도 바꿨다. 모델 티어·별칭 해석은 `provider_registry.resolve_deepseek_model()`이 소유하며 bot_config와 브라우저 두 경로가 공유한다. 옛 `deepseek-v4-pro` ID도 실행 시 Flash로 정규화하며 과거 ID의 비용 행은 보존한다. 현재 에이전트·주기 작업 설정은 `deepseek_flash`를 사용하고, 옛 `deepseek_pro`는 저장값 호환에만 남긴다. Writer는 Flash 항목 하나만 표시하며 옛 Pro 선택은 읽기/설정/실행 경계에서 Flash 선택으로 정규화한다.
...
The browser worker accepts tier names, legacy model aliases such as `deepseek_flash`, and official API model IDs, but normalizes them before sending requests upstream. `config/agent_runtime.json` should prefer official API IDs. Browser-use calls the provider through the LLM proxy and `_AuditedBrowserChatMixin` applies policy/usage audit to every agent step. Native computer mode uses `AuditedAsyncOpenAI.responses.create` through that same proxy.
...
`llm/openai_tool_loop.py` converts Anthropic-style tool definitions to function tools and normalizes malformed tool-call messages. GPT-6 uses Responses API with stateless replay of encrypted reasoning items, function calls, and function outputs; older OpenAI-compatible providers keep Chat Completions. It also handles:
