...
Kimi의 개별 호출이 prompt-side `400 content_filter`/`high risk` 예외 또는 정상 completion의 `finish_reason=content_filter`/`message.refusal`로 거부되면 `openai_tool_loop.py`의 API-call 경계에서 같은 메시지와 도구 스키마를 DeepSeek Pro의 OpenAI-compatible endpoint로 재시도하며, 해당 tool loop의 남은 호출도 DeepSeek에 고정한다. 이 정책과 Kimi의 `reasoning_effort=max`, `max_tokens`, reasoning replay 옵션은 `llm.provider_registry.kimi_openai_tool_options()`가 소유하며 Telegram, public web chat, A2A가 같은 설정을 사용한다. 이 경계는 일반 라운드뿐 아니라 도구/예산 한도 뒤의 forced-final 및 finalization-tool 후속 호출에도 공통 적용되므로 이미 실행한 도구를 처음부터 반복하지 않고 provider별 reasoning/tool replay 규약도 섞지 않는다. 로그의 provider/model과 예산 사용량은 전환 뒤 실제 DeepSeek 응답 모델을 기준으로 기록·계산한다. 잘못된 파라미터·스키마 같은 다른 400은 이 폴백 조건에 포함하지 않는다. DeepSeek credential이 없으면 기존 Kimi 오류 처리가 그대로 적용된다.
