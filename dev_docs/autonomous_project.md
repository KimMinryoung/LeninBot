...
The agent prompt also carries a `report-quality` section (lead with the finding, absolute dates, comparators on every figure, fact/interpretation/forecast register separation with falsifiable indicators, engage counter-evidence, draft from full notes) and a `report-format` section fixing the public-report layout: `## 요약` → `## 핵심 지표` (table) → 2–5 descriptive `##` analysis sections → `## 반론과 한계` → `## 전망` → `## 출처`, body headings `##`/`###` only, no multi-level decimal numbering, body target 6,000–15,000 chars. Three of these are mechanically enforced by `validate_autonomous_research_publication` at publish time (no body H1; `## 요약` present; ≥2 `[^n]:` footnote definitions), with `is_edit=True` relaxing the layout checks (but not the body-H1 check) so factual corrections to pre-layout legacy documents are not blocked. Legacy body-H1 headings in 11 existing documents were demoted via `scripts/demote_research_body_h1.py` (backup `data/publication_drafts/research_body_h1_backup_20260711.json`).
