...
외부 영향력 투사 기능은 티어로 분리 — preferences가 열려있을수록 누적 오정렬 리스크가 크기 때문.
...
CREATE TABLE autonomous_projects (
    ...
    title         TEXT NOT NULL,
    topic         TEXT NOT NULL,
    preferences   TEXT NOT NULL,       -- 내부 디렉티브, 매 tick 프롬프트에 주입
...
systemd timer (*:17 KST)
  └─► python -m autonomous_project
        └─► run_tick()
              ...
              │    ├─ _build_task_prompt()  ← <project> <preferences> <state> <plan> <recent-notes> 주입
...
| 명령 | 용도 |
|------|------|
| `create --title T --topic X --preferences P` | 새 프로젝트 생성 (state=researching) |
| `list` | 전체 목록 (state, turn_count, last_run_at, title) |
| `show <id>` | 단건 상세 — preferences, plan, 최근 5 notes |
| `events <id> [--limit N]` | 이벤트 로그 (기본 30건) |
| `edit <id> [--title T] [--topic X] [--preferences P \| --preferences-file F]` | 필드별 수정 + `project_edited` 이벤트 기록 |
| `pause <id>` / `resume <id>` / `archive <id>` | 상태 전환 (스케줄러 대상에서 제외/포함/종결) |
| `tick` | 수동으로 한 번 즉시 실행 (테스트용) |

preferences가 multi-line일 때는 `--preferences-file /path/to/file.txt` 또는 `--preferences-file -` (stdin) 사용.

### 방향 전환 워크플로우

소폭 조정:
```
pause 1 → edit 1 --preferences ... → resume 1
```

대폭 방향 전환 (기존 notes/plan이 새 방향과 어긋나 혼란을 줄 경우):
```
archive 1 → create --title ... --preferences ... → (새 프로젝트로 시작)
```
...
3. **Preferences 변경 감지 없음**: 사용자가 `edit` 으로 preferences를 바꿔도 agent는 변경 사실 자체를 모른다 — 단지 다음 tick에서 새 텍스트를 읽을 뿐. 누적된 notes/plan이 새 방향과 어긋난 경우 agent가 자각하지 못할 수 있다. **"preferences가 최근 수정됨"** 플래그를 프롬프트에 주입하는 게 개선 방향.
...
- [ ] **preferences_recently_changed 플래그**: `edit` 시 플래그 ON, 다음 tick 프롬프트에 "preferences가 방금 바뀌었다 — 기존 plan·notes와 새 preferences 정합성을 점검하라" 주입, tick 완료 시 OFF.
