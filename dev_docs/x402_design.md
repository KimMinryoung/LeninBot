...
- `pay_and_fetch` 도구로 leninbot이 자기 API의 보호된 라우트에 결제하고 컨텐츠를 받아오는 self-loop 한 사이클 — 사람 개입 없이 전 단계 자동. 초기 성공 가격은 0.001 USDC였고, 현재 기본 demo 가격은 0.05 USDC다.
...
- `/x402-demo/quote` 기본 가격은 `X402_DEMO_QUOTE_USDC` env로 설정하며 기본값은 `0.05` USDC (`50000` atomic units)다.
- `pay_and_fetch` client cap은 `X402_MAX_USDC_PER_CALL` env로 설정하며 기본값은 `0.05` USDC다. Demo 가격을 이 값보다 높이면 기본 도구 호출은 서명을 거부한다.
- Self-loop demo는 payer와 recipient가 같은 지갑이라 USDC 순변동은 0이고 gas만 소비된다. 외부 결제 공개 전에는 가격이 예상 gas와 abuse 비용을 충분히 덮어야 한다.

### Audit Ledger

`crypto_wallet/x402_ledger.py` owns the PostgreSQL audit table `x402_payment_attempts`.
Apply it with `venv/bin/python scripts/schema_migrations.py --only x402-ledger`.

The ledger records inbound demo route events and outbound `pay_and_fetch` attempts:
direction, stage, status, resource, method, payer, recipient, amount, asset, network,
tx hash, gas used, HTTP status, error, and metadata. It does not store signatures,
private keys, credential paths, or raw payment headers.
...
|---|---|
| 메서드 | GET |
| 가격 | 기본 0.05 USDC (= 50000 atomic units), `X402_DEMO_QUOTE_USDC`로 조정 |
...
self-loop 특성상 USDC 순 변동량 = 0, 가스만 소모된다. 외부에서 들어오는 결제로 확장하면 price minus gas가 순수입이 된다. Demo 기본 가격은 작은 gas 변동과 abuse 비용을 덮기 위해 0.05 USDC로 둔다.
...
- **단일 demo 가격**: `/x402-demo/quote`는 `X402_DEMO_QUOTE_USDC` 기반 단일 가격. 컨텐츠 가치별 차등 과금은 아직 없음
...
1. **컨텐츠 다양화 (Phase 1)**
   - `/x402/aphorism` (현재 quote의 이름 정리, 기본 0.05 USDC)
...
3. **운영 인프라**
   - audit log: PostgreSQL `x402_payment_attempts`에 모든 결제 시도 기록 (timestamp, payer, amount, tx_hash, content_id/metadata)
...
- **2026-05-09**: `x402_payment_attempts` audit ledger와 `scripts/schema_migrations.py --only x402-ledger` migration entrypoint 추가. inbound demo route와 outbound `pay_and_fetch`가 best-effort로 결제 시도를 기록.
- **2026-05-09**: `/x402-demo/quote` 가격을 env 기반 `X402_DEMO_QUOTE_USDC`로 전환하고 기본값을 0.05 USDC로 상향. `pay_and_fetch` 기본 cap과 일치시켜 demo는 동작하되 저가 반복 요청의 손실 위험을 줄임.
