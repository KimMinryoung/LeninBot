...
`parse_json_object(text, keys=...)`는 엄격 파싱이 실패할 때만 알려진 키 순서로 문자열을 잘라 복구한다. 일기 #438이 9월 5일부터 매일 밤 같은 자리에서 실패해 추가했다. 복구는 값이 문자열인 평면 객체만 다루며, 중복 키·본문의 키 형태 표현·키 순서 모호성을 거부한다. 복구 성공 로그와 회귀 테스트가 있다. 운영 `research-document-translation.service` 로그에서 2026-09-07 일기 #438의 복구와 영어본 갱신, 2026-09-09 추가 복구를 확인했다. 이 관측은 다른 형식이나 모든 향후 실패에 대한 보증은 아니다.
