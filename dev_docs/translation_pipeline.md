...
Markdown은 최상위 문단·목록·표·코드 블록 경계로 나눈다. 목표 8,000자는 하드 상한이 아니며 초대형 단일 표·목록은 더 클 수 있다. 호출 전에 코드·HTML 태그·링크 목적지·참조 ID를 결정론적 플레이스홀더로 보호하고, 출력의 개수를 검사한 뒤 복원한다. 전체 블록 구조, 모든 링크·이미지 목적지, 참조 정의, 각주 ID, 코드, HTML 구조와 한글 잔존율을 검사한다. 최종 문서 조립 후에도 전체 검증을 통과해야 한다.
