...
- **`audit.py`** — `audit(...)`: recursively redacts+truncates args, emits a
  structured JSON log line, and enqueues a row to `tool_audit_log` via a single
  background worker thread. Queue overflow briefly writes to disk in the caller;
  sink failures spool rows in the worker for retry. Owns
  `ensure_tool_audit_log_table()` (the `tool-audit-log` migration).
...
| `mcp` | Local stdio MCP server; profile allow-list followed by shared authorization and audit | `True` only for repository-owner `operator` |
...
`ops/audit_sink.py` — see `dev_docs/llm_gateway.md` "감사 싱크"). The proxy is the
...
- **Audit is non-fatal.** Sink errors never fail a tool call. Failed DB rows and
  queue overflow are written to `data/audit_spool/` and retried; a spool write
  failure is logged as actual loss. `venv/bin/python scripts/audit_spool_status.py`
  reports outstanding rows by ledger. Replay may produce duplicate rows if a
  process exits after the sink accepted a POST but before deleting its spool row.
...
- MCP operator의 별도 OS 계정·전용 DB credential 분리는 남아 있다. 현재 모든 MCP 호출은 공통 authorization/audit 경계를 지나고, operator는 저장소 소유 UID에서만 선택할 수 있다. SQL은 읽기 전용이며 KG 변경은 별도 확인 문자열과 유효한 사전 백업이 필요하다. 현재 경계는 [mcp_gateway.md](mcp_gateway.md)를 따른다.
- 감사 spool 행의 sink 수락 후 삭제 전 종료 시 중복 기록을 제거하려면 sink의 idempotency key가 추가로 필요하다.
