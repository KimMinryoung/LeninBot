...
허용하며 알 수 없는 ID에는 사용 가능한 ID와 chunk 범위를 반환한다.
여러 비연속 chunk는 각 연속 구간의 별도 인용으로 변환한다. 조사·작성·공통 저장 검증은
근거 개수 상한을 두지 않으며, 개수 때문에 거절하거나 재조사하지 않는다. 독립 검토의 checks도
개수로 거절하지 않는다. 한 chunk의 단수 표기(`chunk`)도 수용한다.
조사는 실제 변경 사실을 뒷받침하면 멈춘다. 출처·주장 개수 목표를 두지 않고 같은 사실의
출처나 인용을 늘리지 않는다. 한 출처를 여러 필드에 재사용하고 추가 검색은 불확실성·상충·누락 해소에 한한다.
조사 주장은 **인용문(`quote`, 20~1000자, 권장)** 또는 표시된 chunk 번호로 근거를 댄다. 인용문은
`commulingo_review_policy.locate`가 따옴표·대시·공백·대소문자를 접은 정규화로 원문에서 찾아 문자
범위로 바꾼다(모델은 번호 매칭보다 인용을 훨씬 잘한다: 2026-09-13~19 chunk/source_id 오류 227건).
같은 `normalize`를 독립 검토의 인용 검사도 쓴다(정확 일치 실패 63건/주). 정규화는 같은 구절의 두
표기를 같게 만드는 치환만 하고, 다른 구절을 같게 만들 수 있는 치환은 하지 않는다.
실행기가 240자 단위 chunk 번호를 정확한 문자 범위로 변환해 기존 research artifact 형식으로
저장한다. 모델에게 문자 위치 계산을 맡기지 않으며 원문 일치·만료·20~6000자 인용 검증은 유지한다.
