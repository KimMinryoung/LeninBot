...
신규 절의 `sortOrder`(YYYYMM)를 작성자가 생략하면 같은 모듈의 `section_sort_order`가 제목의
첫 연도(ko 우선)로 YYYY00을 채우고, 연도가 없으면 기존 마지막 절 뒤(+1)에 둔다. 공유 저장소는
값이 없으면 0을 쓰므로 이 보정이 없으면 절이 맨 앞으로 간다(2026-09-22~24 편집기 절 156건).
