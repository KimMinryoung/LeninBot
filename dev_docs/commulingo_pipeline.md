...
조사기는 field별 주장·호출 내 짧은 source ID(S1, S2)·**출처 표시 텍스트에서 그대로 복사한 인용문
(20~2000자)**을 제출한다. 실행기가 인용문을 스냅샷에서 찾아(굽은 따옴표·줄표·공백·대소문자를
접어서 비교하는 `commulingo_review_policy.locate`; 지정한 출처에 없으면 그 작업의 다른 스냅샷도
찾는다) 정확한 문자 범위로 바꾸고, 앞뒤 문장 경계까지 넓힌 발췌(최대 ±300자)를 근거로 저장한다.
모델에게 번호나 문자 위치를 세게 하지 않으며, 240자 chunk 번호 체계(2026-09-19까지)는 폐기했다 —
표시 번호와 스냅샷 ID가 어긋나 하루 91건이 거절됐고 발췌가 낱말 중간에서 잘렸다. 짧은 ID는 내용
hash 기반 영속 ID로 변환해 artifact에 저장한다. 영속 ID 입력도 허용하며, 그 URL의 현재 스냅샷으로
해석한다. 알 수 없는 ID에는 사용 가능한 ID와 URL을 돌려준다. 조사·작성·공통 저장 검증은
...
인물 create 초안에서 `groupId`·`role`, 용어 create 초안에서 `category`는 작성기가 아니라 실행기가 Jev로 배정한다(`runtime_tools/commulingo_classify.py`,
