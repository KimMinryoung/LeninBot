...
그 텍스트를 기억하므로(`evidence.label_passages`), claim의 라벨을 문자 범위로 바꾸는 데 복사·매칭·세기가 없다
(`evidence.resolve_passages`): 표시된 적 없는 라벨, 두 출처의 라벨 혼용, 표시 뒤 바뀐 텍스트, 20자 미만 제목 문단,
6,000자 초과 범위는 claim 번호를 붙여 거절한다. 인용문 복사+접기 매칭(2026-09-19 하루)과 240자 chunk 번호(그 전)는
