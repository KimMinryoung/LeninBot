...
스냅샷 뒤에 이어 붙고(페이지는 내용 hash로 식별, 텍스트 검색 없음) chunk 번호는 이어진다. 표시에
`Chunks a..b of 0..N`을 적는다. 핸들 S번호도 URL당 하나로 고정된다. 이전 시도에서 남은 같은 URL의
스냅샷들은 조사 시작 시 오래된 순으로 병합한다. 2026-09-13~19 chunk 번호 오류가 난 작업 71개 중
68개가 한 URL에 스냅샷이 여럿이었다(2페이지의 chunk 번호를 1페이지 핸들로 인용). 인용문 문자열
매칭은 두지 않는다.
