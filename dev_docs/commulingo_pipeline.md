...
여러 비연속 chunk는 각 연속 구간의 별도 인용으로 변환하고, 최종 인용 수 50개 상한도 조사 완료 전에 검사한다.
근거를 임의로 잘라내지 않으며 초과 시 같은 조사 호출에서 조정한다. 또한 한 chunk의 단수 표기(`chunk`)도 수용한다.
...
기준 통과 후 다음 배치부터 적용하고 24시간 창의 생산율과 미정산 비용을 확인한다.
