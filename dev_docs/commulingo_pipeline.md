...
상세 절 작성은 첫 호출에 제목·시작연도·과제 판단·사유를 저장하고, 본문의 한국어·영어와 근거는
작은 호출로 각각 저장한다. 일반 한영 서술 필드도 초안 단계에서 언어별 병합을 허용한다.
한 언어만 저장된 상태는 체크포인트에 보존되지만 전체 필드 검증·독립 검토·공개로 넘어갈 수 없다.
부분 제출은 도구 게이트웨이에서 `continued`로 기록하고 모델 도구 루프에 계속 입력한다.
터미널 도구 이름을 호출했더라도 부분 저장만으로 세션을 종료하지 않는다.
2026-09-26에는 이전 경로가 부분 초안 저장 문자열을 정상 터미널 결과로 취급해 세션을 끊었고,
그 뒤 단계가 `stage ended without validated result`로 실패했다. 시도 지표의 `terminal_calls=1`과
체크포인트의 `heading`, `startYear`만 저장된 상태가 이 경로를 확인해 주었다.
OpenAI 호환 응답의 도구 인자 JSON이 깨지면 같은 긴 인자를 재전송하지 않고 언어별로 나눠
저장하라는 오류 안내를 돌려준다. 검토 문맥 조회 도구는 현재 항목에 실제로
있는 필드만 schema enum으로 제시해 존재하지 않는 필드 요청을 줄인다.
편집기는 공용 큐레이터 정책(16라운드)과 별도로 `EDITOR_MAX_ROUNDS`=24라운드를 쓴다(2026-09-25, 12라운드가 실패 32건 중
27건을 끝냈고 평균 비용 $0.0135로 단계 예산 $0.20에 여유가 컸다). 비용 상한은 단계 예산이 그대로 막는다.
아직 빠진 부분이 있는 제출은 거절이 아니다. 초안에 저장하고 `Still needed before validation: …`을 돌려주며
