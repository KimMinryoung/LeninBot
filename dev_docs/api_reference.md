...
`api.py` includes small route modules for separated API surfaces: `api_routes/email.py` owns `/email/*`, `api_routes/task_reports.py` owns `/reports*`, `api_routes/private_reports.py` owns `/private-reports*`, and `api_routes/x402_demo.py` owns `/x402-demo/quote`.
