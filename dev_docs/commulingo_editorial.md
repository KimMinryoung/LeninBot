...
- 검토기는 checks[].citation_id(S1=source_refs[0])와 자신이 가져온 source_id(R…, 또는 그 URL), 그리고 가져온 원문에서 **그대로 복사한 인용문(20~1000자)**을 낸다. 실행기가 인용문을 굽은 따옴표·줄표·공백·대소문자를 접어 찾고(지정 출처에 없으면 이 검토에서 가져온 다른 출처도 찾는다) 그 자리의 원문 그대로를 citation/source/quote/finding 형식으로 저장한다. 행 번호 체계(line_start/line_end, 240자 표시 분할)는 2026-09-19에 폐기했다 — 주당 87건이 「인용이 원문에 없음」, 76건이 행 범위 오류로 떨어졌다. 원문은 현재 검토에서 직접 가져온 것만 인정한다. 못 찾은 check는 결정 전체를 거절하지 않고 그 check만 뺀 뒤 `dropped_checks`(번호·사유·인용 머리)로 저장 결정에 남긴다 — 찾은 check가 하나도 없을 때만 거절(2026-09-19: 9~39개 check 중 하나가 어긋나면 결정 전체가 사유 표시 없이 떨어져 하루 159건 재제출). 접두 폴백(40자)은 조사 레인과 같다. resolved_risks는 risks 식별자만 담고 설명은 reason/finding에 쓴다.
