...
자동 Editor는 직접 저장 도구와 별개로 세션 전용 typed API를 사용한다.
`changes.<field>`에 `value`와 원문 문단을 인용하는 `evidence`를 함께 받으며,
부분 수정도 같은 형태로 해당 필드의 값과 근거를 함께 교체한다. 과제별 판단은 ID로 연결하고,
무편집 종료는 별도 도구로 받는다. 상세 형식은 [필드 단위 작성 API](commulingo_pipeline.md#필드-단위-작성-api와-체크포인트)를 따른다.
`author_draft.py`가 이를 기존 저장 계약으로 변환하므로 아래 직접 도구의 호출 형식,
frontend RPC·revision·독립 검토·원자적 공개 검증은 유지한다.

인물 상세 절의 직접 저장 도구는 신규 등록에서 slug를 받지 않는다. Python API 경계가
`commulingo_section_slug` 경량 원샷 호출로 한영 제목·본문에서 주제 slug를 생성한 뒤
공통 저장 서비스에 전달한다. 기존 절 수정은 `slug`를 지정하며 자동 생성하지 않는다.
정렬 키도 작성자가 인코딩하지 않는다. `start_year`(필수, 시기 없음은 null)와 `start_month`를
받아 서버가 `sortOrder`=YYYYMM을 만들고, null이면 신규는 마지막 절 뒤에 붙이고 수정은 위치를 유지한다.
모델 출력은 ASCII kebab 형식으로 접은 뒤 형식·인물 ID·기존 절 충돌을 검증한다. 호출 실패나
부적합한 결과는 영문 제목에서 결정적 slug(최대 48자, 충돌 시 `-2` 접미)로 대체하며, 저장을
막는 것은 기존 절과 같은 제목(`_dedup_key` 기준)뿐이다.
